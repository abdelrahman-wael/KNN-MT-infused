...
class TranslationMultiSimpleEpochTask(FairseqTask):
    ...
    def build_dataset_for_inference(self, src_tokens, src_lengths,tgt_tokens=None,tgt_lengths=None):
        ...
        if tgt_tokens:
            tgt_data = ListDataset(tgt_tokens, tgt_lengths)
            dataset = LanguagePairDataset(src_data, src_lengths, self.source_dictionary,tgt=tgt_data,tgt_sizes=tgt_lengths)
        else:
            dataset = LanguagePairDataset(src_data, src_lengths, self.source_dictionary,shuffle=False)
        ...
        with torch.no_grad():
            ...
            if not self.args.lang_tok_replacing_bos_eos:
                ...
                return generator.generate(
                        models,
                        sample,
                        prefix_tokens=prefix_tokens,index=index
                )
            else:
                return generator.generate(
                        models,
                        ...
                        if tgt_langtok_spec else self.target_dictionary.eos(),index=index
                )
