...
class KNN_Dstore(object):
    def __init__(self, args):
        self.args=args
        self.args.average_dist_per_batch=np.array([0])
        self.args.average_dist=np.array([0])
        self.half = args.fp16
    ...
    def setup_faiss(self, args):
        ...
        print("no longer needed ds created on the fly")
        # if args.knn_q2gpu:
    ...
    def adaptive_dist(self,dists,dist_temp):
        # 
        alpha_vec =torch.relu(1-(dists[:,0]/dist_temp))
        # adapt_dist[adapt_dist==0]=100000
        alpha_vec[alpha_vec==0]=0.00000001
        return alpha_vec
    ...
    def get_knn_scores_per_step(self, queries, vocab_size, pad_idx, use_dtype=torch.float32, save_knns=False, knn_temp=1.0):
        ...
        def get_knns_brute():
            ...
            self.args.ds_hypos
        self.k = self.args.all_ds_size[0] if self.args.all_ds_size[0] <2048 else 2047
        all_dists, all_knns = self.get_knns(queries)
        self.k = self.args.k
        #print(f"Querying knns: {time.time()-start_time}s")
        # (Bxbeam_size)xK
        # self.sentence[""]
        dists_numpy = dists.copy()
        knns_numpy = knns.copy() 
        dists = torch.from_numpy(dists).type(dtype=use_dtype).cuda()
        dists_save = dists.clone()
        # self.args.average_dist_per_batch.append(dists.mean())
        self.args.average_dist_per_batch=np.append(dists.cpu().mean(),self.args.average_dist_per_batch)
        self.args.average_dist=self.args.average_dist_per_batch[:-1].mean()
        # old_dist=dists
        adaptive_lambda=self.adaptive_dist(dists,dist_temp=self.args.adaptive_temperature)
        #print(dists)
...
#                full_knn_scores[i, j] = torch.logsumexp(probs[i, locs].view(-1), dim=0)
        ## FOR DEBUGGING
        if save_knns:
            return full_knn_scores, [torch.from_numpy(knns).long().cuda(), probs.squeeze(-1), indices.squeeze(-1), dists_save.squeeze(-1)], adaptive_lambda 

        return full_knn_scores ,None ,adaptive_lambda , all_dists, all_knns
