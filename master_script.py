...
if args.save_job:
    ...
    with executor.batch():
        for dataset_idx, curr_save_data in enumerate(args.save_data):
            ...
            save_cmd = f"python fairseq_cli/generate.py {curr_save_data} --gen-subset train --path {args.model} --beam 5 --remove-bpe --bpe {args.bpe} --num-workers 35 --max-sentences 2 -s de -t en --source-lang de --target-lang en --tokenizer moses --moses-source-lang de --moses-target-lang en --sacrebleu --score-reference --dstore-mmap {curr_save_subset_mmap} --knn-keytype last_ffn_input  --model-overrides {{\'knn_keytype\':\'last_ffn_input\'}} --save-knn-dstore --save-knn-subset --save-knn-subset-num {args.num_for_training} "
            print(save_cmd)
    ...
    generating_embedding_time = end-start
    print("time taken to generate embedding "+str(generating_embedding_time))

# SAVE KEYS/VALUES SUBSET FOR TRAINING

# MERGE SUBSET KEYS/VALUES
if args.merge_dstore_job:
    print("Merging Subset ")
    start = time.time() 
    ...
    merge_subset_time = end-start
    print("time taken to merge subset "+str(merge_subset_time))
...
#             train_cmd = f"python train_index.py --dstore_mmap {dstore_mmap} --dstore_size {size} --dimension 512 --code_size {args.code_size} --ncentroids {ncentroid} --train_index {train_index} --from_subset --gpu"
#             print(train_cmd)
...
#     end  = time.time() 
    #   merge_subset_time = end-start
    #   print("time taken to train indexes "+str(merge_subset_time))
# # TRAIN INDEX
...
if args.add_keys_job:
    ...
    with executor.batch():
        ...
        for dataset_idx, (curr_save_data, curr_dstore_size, num_shards) in enumerate(zip(args.save_data, dstore_size, args.num_shards)):
            ...
            for shard_idx in range(num_shards):
                ...
                add_cmd = f"python fairseq_cli/generate.py {curr_save_data} --gen-subset train --path {args.model} --beam 5 --remove-bpe --bpe {args.bpe}  --tokenizer moses -s de -t en --moses-source-lang de --moses-target-lang en --sacrebleu --score-reference --knn-keytype last_ffn_input  --model-overrides {{\'knn_keytype\':\'last_ffn_input\'}} --num-workers 35 --save-knn-dstore --knn-add-to-idx --num-shards {num_shards} --shard-id {shard_idx} {train_index} {write_index}"
    ...
    merge_subset_time = end-start
    print("time taken to add keys  "+str(merge_subset_time))
# Add keys to an already trained index.
...
if args.merge_index_job:
    ...
    merge_subset_time = end-start
    print("time taken to MERGE FAISS INDICES "+str(merge_subset_time))
# MERGE FAISS INDICES
